...
class CouponController extends Controller
{
    ...
    public function apply(Request $req)
    {
        ...
        // Si ya hay uno en sesión, no dupliques
        ...
        // Necesitamos el subtotal actual del carrito
        $oldCart = Session::get('cart');
        if (!$oldCart) return response()->json(['ok'=>false,'msg'=>'Tu carrito está vacío.'], 409);
        $cart = new Cart($oldCart);
        $subtotalCents = (int) round($cart->totalPrice * 100);
        ...
        return DB::transaction(function() use ($data, $sessionId, $ip, $ua, $userId, $subtotalCents) {
            ...
            // límite por usuario (si usas por_usuario > 0)
            ...
            // stock disponible = emitidos - reclamados - reservas activas
            ...
            // descuento calculado sobre subtotal (si tienes bandera aplica_solo_subtotal=true ya estamos usando subtotal)
            $descuentoCent = $cupon->calcularDescuento($subtotalCents);
    ...
    /** POST /cupones/validate : consulta/valida el estado actual de la reserva */
    ...
    /** DELETE /cupones/remove : liberar el bloqueo */
    ...
    /**
     * POST /cupones/finalize : canje definitivo tras pago exitoso.
     * Si prefieres, puedes NO usar esta ruta y hacer el canje directo en tu MercadoPagoController (abajo te indico dónde).
     */
    public function finalize(Request $req)
    {
        $ventaId = $req->input('venta_id'); // opcional
        $rid = Session::get('cupon_reserva_id');
