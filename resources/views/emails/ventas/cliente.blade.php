@php
    ...
    $whatsapp = env('SALES_WHATSAPP', '+51 900 000 000');
...
<head>
    <meta charset="UTF-8">
    <title>Compra exitosa - Adler</title>
    <meta name="color-scheme" content="light">
    <style>
        /* mismo CSS que el admin, solo cambiaremos 2 textos */
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .header{background:linear-gradient(135deg,#8B0000 0%,#B22222 100%);padding:38px 20px;text-align:center;}
        .logo{font-size:32px;font-weight:300;letter-spacing:3px;color:#fff;}
        .logo span{font-style:italic;}
        .content{padding:48px 38px;}
        .confirmation-text{font-size:15px;color:#666;margin-bottom:36px;text-align:center;}
        ...
        .client-detail strong{color:#333;font-weight:500;display:inline-block;min-width:90px}
        ...
        .total-section{text-align:right;padding-top:26px;border-top:2px solid #f0f0f0;margin-top:14px;}
        .total-label{font-size:12px;text-transform:uppercase;letter-spacing:1px;color:#999;margin-bottom:6px}
        ...
        @media(max-width:600px){
            ...
            .info-item{flex-direction:column;align-items:flex-start;gap:3px}
        }
    </style>
</head>
<body>
<div class="email-container">
    <div class="header">
        <div class="logo"><span>A</span>dler</div>
    </div>

    <div class="content">
        <p class="confirmation-text">Gracias por tu compra. Hemos recibido tu pedido.</p>
        ...
        <div class="client-info">
            ...
            @if($isDelivery)
                ...
                <div class="client-detail"><strong>Dep.:</strong> {{ $cliente['envio']['departamento'] ?? '-' }}</div>
                <div class="client-detail"><strong>Costo envío:</strong> {{ $mon($venta['cargo_envio'] ?? 0) }}</div>
            @else
                <div class="client-detail">
                    <strong>Recojo:</strong> coordina al WhatsApp {{ $whatsapp }} indicando tu código {{ $venta['codigo'] ?? '' }}.
                </div>
            @endif
        </div>
            ...
            <table class="products-table">
                ...
                @foreach($lineas as $l)
                    <tr>
                        ...
                        <td style="text-align:right;">{{ $mon($l['subtotal'] ?? 0) }}</td>
                    </tr>
                @endforeach
                </tbody>
            </table>
        ...
        <div class="total-section">
            ...
            <div class="total-amount">{{ $mon($venta['total'] ?? 0) }}</div>
        </div>

        <div class="note">
            Si tienes dudas responde este correo o escríbenos al WhatsApp {{ $whatsapp }}.
        </div>
    </div>

    <div class="footer">
        <div class="footer-title">Adler • Mochilas de emergencia</div>
        ...
        <div class="footer-text">
            {{ date('Y') }} Adler. Gracias por no abandonar el carrito.
        </div>
    </div>
</div>
</body>
