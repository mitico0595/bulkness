        ...
        <div class="progress-container">
            <div class="progress-steps">
                <div class="step completed">
                    ...
                    <a href="{{asset('adler-venta-detalle')}}" style="z-index:99"><div class="step-label">Carrito</div></a>
                </div>
                <div class="step completed">
                    <div class="step-circle">✓</div>
                    <a href="{{asset('delivery')}}" style="z-index:99"><div class="step-label">Envío</div></a>
                </div>
            ...
            <div class="right-section">
                <div class="order-summary">
                    ...
                    <div class="summary-totals">
                        ...
                        <div class="summary-row">
                          <span>Descuento</span>
                          <span style="color: #f44336;" id="cardDescuento">
                              - S/. {{ number_format((($coupon_discount_cents ?? 0)/100), 2) }}
                          </span>
                        </div>
...
async function renderPaymentBrick() {
  const settings = {
    initialization: {
      amount: Number(Number(@json($amount)).toFixed(2)),
    ...
    callbacks: {
      ...
      onSubmit: ({ selectedPaymentMethod, formData }) => {
        ...
        const pagoPromise = fetch(@json(route('mp.process')), {
          method: 'POST',
  ...
  loaderUI.start();
    ...
    try {
      const pagoPromise = (async () => {
        ...
        const res = await fetch("{{ route('mp.yape') }}", {
          method: "POST",
          headers: {
...
(function(){
  ...
  // 1) Preguntar al backend si hay cupón, igualito que en tu otro blade
  fetch("{{ route('cupon.validate') }}", {
    method: "POST",
    headers: {
      "X-CSRF-TOKEN": CSRF,
  ...
  .then(r => r.json())
  .then(j => {
    // j viene con la misma forma que en tu otro blade
    // p.ej. { active: true, code: "...", discount: 1200, new_total: 15900, expires_at: "2025-10-30T16:41:00.000000Z" }
    if (!j || !j.active) {
      // no hay cupón → nos aseguramos de mostrar descuento 0
      if (descEl) descEl.textContent = "- S/. 0.00";
      return;
    }

    // 2) Mostrar DESCUENTO
    let discountSoles = rawDiscount > 100 ? (rawDiscount / 100) : rawDiscount;

    // 3) Mostrar TOTAL ACTUALIZADO si el backend ya lo manda
    if (totalEl && j.new_total) {
      // misma regla: si viene 6291 es centavos
      let totalSoles = j.new_total > 100 ? (j.new_total / 100) : j.new_total;
      totalEl.textContent = "S/. " + totalSoles.toFixed(2);
    }

    // 4) Mostrar TIMER si hay expires_at
    if (j.expires_at && wrap && timerEl) {
      startTimer(j.expires_at);
    } else if (timerEl && timerEl.dataset.expires) {
      // plan B: el controller ya lo había mandado en el HTML
      startTimer(timerEl.dataset.expires);
    }
  })
  .catch(() => {
    // si falla la validación, pero el controller dejó data-expires en el HTML, usamos esa
    if (timerEl && timerEl.dataset.expires) {
      startTimer(timerEl.dataset.expires);
    }
  });

  // ======== la misma función que usas en el otro blade ========
  function startTimer(iso) {
    const end = new Date(iso).getTime();
    if (!wrap || !timerEl || !end) return;
    ...
    const intv = setInterval(() => {
      ...
      if (diff <= 0) {
        clearInterval(intv);
        // cuando expires, haz lo mismo que hacías en el otro blade:
        // quitar cupón en backend y recargar
        handleCouponExpired();
      }
    }, 1000);
  }

  function handleCouponExpired() {
    fetch("{{ route('cupon.remove') }}", {
      method: "DELETE",
      headers: { "X-CSRF-TOKEN": CSRF }
    })
    .finally(() => {
      // mostramos un modal chiquito 3s y recargamos
